...
| `notifications.route` | ! no menu bar app is connected and listening for events, so notifications fall back to osascript (shown as "Script Editor") | `open -a "CC Supervisor"` |
...
- To run it by hand and watch its log, stop the LaunchAgent first (`ccs daemon stop`), then run `ccs daemon run --foreground`. A second copy you start by hand exits immediately with `daemon already running`, even if `daemon.lock` was deleted: the running copy still answers on the socket, and it recreates the lock file within a second. The launchd copy instead waits (log: "another daemon holds … waiting for it to exit") and takes over when yours exits.
...
| `ccs statusline apply` reports a conflict after a config dir change | You changed `statusLine` in the old dir's `settings.json` after applying | Restore the old `statusLine` by hand if you want it (the value is in `~/.local/state/ccs/statusline/<profile>.json`), delete that file, then apply again. |
| `ccs statusline apply` or `revert` fails with `settings_busy` | Claude Code kept rewriting `settings.json` while ccs was updating it; nothing was written | Run the command again. |
...
| Widgets stay on gray loading skeletons, or an Edit Widget option doesn't work, right after an update | macOS is still running the previous widget code | `make upgrade` restarts it automatically; for a manual fix run `killall CCSupervisorWidgets` (macOS relaunches it) |
...
| Menu (and Settings → Advanced → Widgets) warns that widgets can't read the supervisor's data | The daemon was installed with `CCS_STATE_DIR` or `XDG_STATE_HOME` set, so its state folder isn't `~/.local/state/ccs`, the only folder widgets may read | Unset the variable, then `ccs daemon install` (same fix as the `state.dir` doctor check) |
