...
if __name__ == "__main__":
    try:
        ...
        _p = CONSTANTS.get("profile") or {}
        _t = (str(_p.get("emoji") or "") + " " + str(_p.get("name") or "")).strip()
        print((_t + " ~ " if _t else "") + "?%")
        raise SystemExit(0)
...
def render_script(profile: Profile, config: Config, *, python: str | None = None) -> str:
    ...
    lines = [
        ...
        f"# Run as: {statusline_command(script_path(profile), python)}",
        "",
        f"CONSTANTS = {constants_for(profile, config)!r}",
        "",
        "_SOURCES = {",
    ]
